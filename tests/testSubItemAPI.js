...
.then(function() {
    data.subItem.text = "superText"
    return rp({
        method: 'PUT',
        uri: 'http://localhost:8080/rest/items/' + data.item.id + '/subitems/' + data.subItem.id,
        body: data.subItem,
        jar: cookie,
        json: true
    })
    .then(function(subItem) {
        if (subItem.text !== "superText") {
            throw Error('Change not successful');
        }
    })
    .then(log('Update subItem OK'));
})
.then(function() {
    return rp({
        method: 'GET',
